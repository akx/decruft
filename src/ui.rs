    ...
    pub fn select_next_or_previous(&mut self, filtered_dirs: &[CruftDirectory], forward: bool) {
        ...
        let new_pos = match current_pos {
            Some(current_pos) => {
                ...
                ((current_pos as i64) + if forward { 1 } else { -1 }).max(0).min(list_len) as usize
            }
            None => 0
        };
...
fn filter_dirs(
    ...
    let mut filtered = dirs
        ...
        .filter(|dir| {
            ...
            if !(show_all_types || is_common_cruft(&dir.crufty_reason)) {
                return false;
            }
                ...
                match &app_state.confirm_delete {
                    ...
                    None => {
                        match key.code {
                            ...
                            KeyCode::Char('j') | KeyCode::Down => app_state.select_next_or_previous(&filtered_dirs, true),
                            KeyCode::Char('k') | KeyCode::Up => app_state.select_next_or_previous(&filtered_dirs, false),
